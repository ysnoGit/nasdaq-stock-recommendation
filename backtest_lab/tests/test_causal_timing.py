...
class CausalTimingTest(unittest.TestCase):
        ...
        con.executemany(
            ...
            [
                (
                    ...
                    date(2026, 5, 27), 105, 105, 101, 100, 100,
                ),
                (
    ...
    def test_friday_signal_uses_same_day_completed_week_for_g(self):
        con = duckdb.connect()
        con.execute(
            """
            CREATE TABLE daily (
        ...
        con.execute(
            ...
            ('2026-05-29', '1', '01', 'TEST', 'Test Company', 100, 100, 3,
             100, 100, 100, '2026-06-01', 101, 101, 101, 100, 100)
            """
        )
        con.execute(
            """
        ...
        self.assertEqual(rows[0][4], date(2026, 5, 29))
        self.assertEqual(rows[0][5], date(2026, 6, 5))
        self.assertEqual(rows[0][6], date(2026, 6, 5))
        self.assertEqual(rows[0][11], 110.0)

    def test_clustered_future_rows_without_crossovers_do_not_confirm_f_or_h(self):
        con = duckdb.connect()
        con.execute(
            """
            CREATE TABLE daily (
        ...
        con.execute(
            """
            INSERT INTO daily VALUES
            ('2026-05-27', '1', '01', 'TEST', 'Test Company', 100, 100, 3,
             100, 100, 100, '2026-05-28', 101, 101, 100, 100, 100)
            """
        )
        con.execute(
            """
        ...
        con.execute(
            """
            INSERT INTO weekly VALUES
            ('2026-05-29', '1', '01', 102, 100, 100, 100,
             '2026-06-05', 103, 100, 100, 100)
            """
        )
        con.execute(
            """
        ...
        parameter = {
            "parameter_set_id": 2, "start_date": date(2026, 5, 27),
            "end_date": date(2026, 6, 5), "annual_growth_pct": 2,
            "quarterly_growth_pct": 2, "annual_years": 2, "quarter_count": 2,
            "volume_ratio_threshold": 2, "volume_surge_min_days": 1,
            "daily_ma_tolerance_pct": 1, "weekly_ma_tolerance_pct": 2,
        }

        self.assertEqual(evaluate_parameter(con, parameter).fetchall(), [])
